...
package object cmd {
  ...
  def loadModel(path: String): Seq[FullDependency] = {
    val s =
      new scala.reflect.io.File(new java.io.File(path))(Codec.UTF8).slurp()
    s.parseJson.convertTo[Seq[FullDependency]]
  }

  /** Load a Sculpt model JSON file and return the graph. If classMode is true, convert detailed
    * dependencies to aggregated class-level dependencies.
    */
  ...
  def saveModel(m: Seq[FullDependency], path: String): Unit =
    new scala.reflect.io.File(new java.io.File(path))(Codec.UTF8).writeAll(
      FullDependenciesPrinter(m.toJson))
