...
object Cycles {
  ...
  /** Cycles in the graph, in (a not-uniquely-determined) reverse topological order. The ordering
    * property means that edges never exist from earlier-listed to later-listed cycles. Even
    * one-node "cycles" are included in the output.
    */
  def cycles(nodes: Nodes): Vector[Cycle] =
    (new Components)(nodes)(_.edgesOut.map(_.to))

  /** Human-readable report of cycles (size > 1) in the graph, in descending order by size.
    */
